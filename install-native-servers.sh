...
# Generate self-signed certificate for Tomcat (PKCS12 format)
sudo openssl req -x509 -nodes -days 365 -newkey rsa:2048 \
...
sudo chmod 600 /etc/ssl/webservers/tomcat/tomcat.key
sudo chmod 644 /etc/ssl/webservers/tomcat/tomcat.crt
sudo chmod 600 /etc/ssl/webservers/tomcat/tomcat.p12
...
sudo useradd -r -s /bin/false tomcat-user || true
...
sudo systemctl is-active tomcat10 && echo "✅ Tomcat is running" || echo "❌ Tomcat failed to start"
...
echo "Tomcat:  /etc/ssl/webservers/tomcat/"
echo ""
echo "✨ Installation complete! All services are running natively on the OS."
